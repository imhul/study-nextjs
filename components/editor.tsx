...
export function Editor(props: EditorProps) {
    const { action, editType, post, comment, comments } = props
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        if (!result.ok) {
            toast.error(`Failed to update ${editType} ${data?.id}`, {
                description: "Sorry, we couldn't submit your changes. Please try again later.",
            })
        } else {
    ...
    return (
        <Drawer>
            <DrawerTrigger asChild>
                <Button variant="outline" className="w-16">Edit</Button>
            </DrawerTrigger>
            <DrawerContent>
                <div className="mx-auto w-full max-w-md">
                    <DrawerHeader className="my-8">
                        <DrawerTitle className="capitalize text-2xl">Edit {editType} #{data?.id}</DrawerTitle>
                        ...
                        <div className="flex items-center justify-center space-x-2">
                            <Form {...form}>
                                <form
                                    ...
                                    className="space-y-8" id="editor-form"
                                >
                                    <FormField
                                            ...
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Textarea
                                                        ...
                                                        placeholder="Write something..." {...field}
                                                    />
                                                </FormControl>
                                                <FormDescription>
